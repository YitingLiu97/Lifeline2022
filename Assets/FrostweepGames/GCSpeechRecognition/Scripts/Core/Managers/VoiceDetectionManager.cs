...
namespace FrostweepGames.Plugins.GoogleCloud.SpeechRecognition
{
    public class VoiceDetectionManager : IService, IVoiceDetectionManager
    {
       ...
       /* public void DetectThresholdValue(int durationSec = 0)
        {
            GCSpeechRecognition.Instance.StartCoroutine(_mediaManager.OneTimeRecord(durationSec, GetDetectionValue()));
        }

        public Action<float[]> GetDetectionValue()
        {
            return (samples) =>
            {
                float accum = 0f;
